...
architecture rtl of load_store_unit_to is
    ...
    signal R_sq_valid : std_logic_vector(SQ_ENTRIES - 1 downto 0);
...
begin
    ...
    sq_enqueue <= '1' when sq_full = '0' and
                           uop_in.exec_unit_id = 1 and
                           uop_in.funct(3) = '1' and
                           uop_in.valid = '1' and
        ...
        if rising_edge(clk) then
            if reset = '1' then
                ...
                if sq_enqueue = '1' then
                    R_sq_valid(to_integer(R_sq_tail)) <= '1';
                    for i in 0 to MAX_SPEC_BRANCHES - 1 loop
                        M_sq_valid_snapshot(i)(to_integer(R_sq_tail)) <= '1';
                    end loop;
                ...
                if sq_dequeue = '1' then
                    ...
                    R_sq_valid(to_integer(R_sq_head)) <= '0';
                end if;
    ...
    --              LOAD QUEUE
    -- ======================================
    lq_enqueue <= '1' when lq_full = '0' and
                           uop_in.exec_unit_id = 1 and
                           uop_in.funct(3) = '0' and
                           uop_in.valid = '1' and
                           not (cdb_in.valid = '1' and cdb_in.branch_mispredicted = '1') and
                           stall_in = '0' else '0';
    ...
    process(lq_head_uop, R_load_data, R_load_valid)
    begin
        cdb_out <= UOP_ZERO;
